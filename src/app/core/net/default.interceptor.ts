...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  private handleSuccessData(ev: HttpResponseBase, req: HttpRequest<any>, next: HttpHandler): Observable<any> {
    ...
    switch (ev.status) {
      case 200:
        ...
        if (ev instanceof HttpResponse) {
          const body = ev.body;
          const status = body.code || 200;
          const statusWhiteList = website.statusWhiteList || [];
          const message = body.msg || '未知错误';
          ...
          // 如果请求为非200否者默认统一处理
          if (status !== 200) {
            this.injector.get(NzMessageService).error(message);
            return throwError(ev);
          }
          return of(ev);
        }
  ...
  private handleErrorData(err: HttpErrorResponse, req: HttpRequest<any>, next: HttpHandler): Observable<any> {
    if (err.status === 401) {
      if (this.refreshTokenType === 're-request') {
        return this.tryRefreshToken(err, req, next);
      }
      this.toLogin();
    } else if ([403, 404, 500].indexOf(err.status) > -1) {
      this.injector.get(NzMessageService).error(err.error.msg || '未知错误');
    ...
    return next.handle(newReq).pipe(
      mergeMap((ev) => {
        ...
        if (ev instanceof HttpResponseBase) {
          return this.handleSuccessData(ev, newReq, next);
        }
        // 若一切都正常，则后续操作
        return of(ev);
      }),
      catchError((err: HttpErrorResponse) => {
        return this.handleErrorData(err, newReq, next);
      }),
    );
  }
}
